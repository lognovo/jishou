...
public class CircleFlowIndicator extends View implements FlowIndicator,
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		...
		// Retrieve the radius
		radius = a.getDimension(R.styleable.CircleFlowIndicator_radiusx, 4.0f);
		...
		mCentered = a.getBoolean(R.styleable.CircleFlowIndicator_centeredx, false);
	...
	protected void onDraw(Canvas canvas) {
		...
		int longPaddingBefore,longPaddingAfter;
	    float threeRadius = radius * 3;
		
		longPaddingBefore = getPaddingLeft();
        longPaddingAfter = getPaddingRight();

		if (mCentered) {
			centeringOffset += ((getWidth() - longPaddingBefore - longPaddingAfter) / 2.0f) - ((count * threeRadius) / 2.0f);
        }
